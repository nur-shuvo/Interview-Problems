...
//total probability = gun kore jog
...
void go_to(int curr_node, float probab, int num_traversals){
    if(num_traversals==0){
    ...
    for(int i=0; i<N; i++){
        if(adj_mat[curr_node][i]>0){
            go_to(i, probab*adj_mat[curr_node][i], num_traversals-1);
        }
    }
}
...
void solution(){
    ...
    for(int i=0; i<N; i++){
        if(adj_mat[start][i]>0)
            go_to(i, adj_mat[start][i], num_traversals-1);
    }
